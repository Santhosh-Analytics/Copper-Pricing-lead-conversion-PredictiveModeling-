...
st.markdown(
    "<h1 style='text-align: center; font-size: 38px; color: #55ACEE; font-weight: 700; font-family: inherit;'>Streamline Your Copper Business: Pricing & Lead Management</h1>",
    unsafe_allow_html=True,
)
...
if selected == "About":
    st.markdown(
        "<h3 style='font-size: 30px; text-align: left; font-family: inherit; color: #FBBC05;'> Overview </h3>",
        unsafe_allow_html=True,
    )
    st.markdown(
        """<p style='text-align: left; font-size: 18px; color: #ffffff; font-weight: 400; font-family: inherit;'>
    ...
    st.markdown(
        "<h3 style='font-size: 30px; text-align: left; font-family: inherit; color: #FBBC05;'> Models </h3>",
        unsafe_allow_html=True,
    )
    st.markdown(
        """<p style='text-align: left; font-size: 18px; color: #ffffff; font-weight: 400; font-family: inherit;'>
    ...
    st.markdown(
        "<h3 style='font-size: 30px; text-align: left; font-family: inherit; color: #FBBC05;'> Contributing </h3>",
        unsafe_allow_html=True,
    )
...
if selected == "Predictor":
    ...
    with tab:
        ...
        col1, col, col2, col0, col3 = st.columns([2, 0.2, 2, 0.2, 2])
        ...
        with col1:
            cust = st.number_input(
                ...
                max_value=39999999,
                value=None,
                help="Enter Customer number. Min value is 30000000 and max value is 39999999",
                placeholder="Enter Customer number. Min value is 30000000 and max value is 39999999",
            )
            item_date = st.date_input(
                "Item Date",
                help="Enter product/order date, must be today or within the past six months",
                value=None,
            )
            item = st.selectbox(
                "Select Item Type",
                item_type_options,
                index=None,
                help="Select Item type from the dropdown menu",
                placeholder="Select Item type from the dropdown menu",
            )
            country = st.selectbox(
                "Select Country Code",
                country_options,
                index=None,
                placeholder="Select Country Code from the list",
            )
        ...
        with col2:
            ...
            thick = st.number_input(
                ...
                max_value=7.0,
                value=None,
                help="Min value is 0.10 and max value is 7.",
                placeholder="Min value is 0.10 and max value is 7.",
            )
            width = st.number_input(
                ...
                max_value=2000,
                value=None,
                help=" Min value is 500 and max value is 2000",
                placeholder="Min value is 500 and max value is 2000",
            )
            qty = st.number_input(
                "Quantity in tons",
                min_value=1.00,
                max_value=800.00,
                value=None,
                help=" Min value is 1.00 and max value is 800.00",
                placeholder="Min value is 1.00 and max value is 800.00",
            )
        ...
        with col3:
            del_date = st.date_input(
                "Delivery Date",
                help="Enter product/order date, must be today or within the next six months",
                min_value=item_date,
                value=None,
            )
            price = st.number_input(
                ...
                min_value=10.00,
                value=None,
                help="Enter selling price in $. Min value is 10.00.",
                placeholder="Enter selling price in $. Min value is 10.00.",
            )
            app = st.selectbox(
                "Select Application",
                application_options,
        ...
        volume = (
            float(qty) * float(thick) * float(width)
            if (qty is not None and thick is not None and width is not None)
            else None
        )
        unit_price = (
            float(price) * float(qty) * float(thick)
            if (price is not None and qty is not None and thick is not None)
            else None
        )

        qty_box = (
            transform_single_value(qty, lambda_dict["quantity_tons"])
            if qty is not None
            else None
        )
        thick_box = (
            transform_single_value(thick, lambda_dict["thickness"])
            if thick is not None
            else None
        )
        width_box = (
            transform_single_value(width, lambda_dict["width"])
            if width is not None
            else None
        )
        volume_box = (
            transform_single_value(volume, lambda_dict["volume"])
            if volume is not None
            else None
        )
        unit_price_box = (
            transform_single_value(unit_price, lambda_dict["unit_price"])
            if unit_price is not None
            else None
        )
        price_box = (
            transform_single_value(price, lambda_dict["selling_price"])
        ...
        def check_conditions():
            return (
                ...
                and country is not None
                and del_month_class is not None
                and del_year_class is not None
                and item_day_class is not None
                and item_month_class is not None
                and item_year_class is not None
                and width_box is not None
                and volume_box is not None
                and unit_price_box is not None
                and app is not None
                and prod is not None
                and thick_box is not None
                and thick > 0
                and width_box is not None
                and width > 0
                and qty_box is not None
                and qty > 0
                and price_box is not None
                and price > 0
            )

        data_final = (
            np.append(data, Item_transform, axis=1) if check_conditions() else None
        )
        # st.write(data_final)
    ...
    if button and check_conditions():
        data_final = (
            np.append(data, Item_transform, axis=1) if data is not None else None
        )
        # st.write(data_final)
    ...
    with tab1:
        ...
        reg_col1, col0, reg_col2, col1, reg_col3 = st.columns([1, 0.1, 1, 0.1, 1])

        with reg_col1:
            reg_item_date = st.date_input(
                "Item Date", help="Enter product/order date", value=None, key="item_dt"
            )
        ...
        with reg_col3:
            reg_del_date = st.date_input(
                "Delivery Date",
                help="Enter delivery date",
                min_value=reg_item_date,
                value=None,
                key="del_dt",
            )
            status = st.selectbox(
        ...
        item_type_label = (
            item_type_mapping[reg_item]
            if reg_item in item_type_mapping
            else item_type_label
        )
        ...
        del_time = (
            abs((reg_del_date - reg_item_date).days)
            if reg_del_date is not None
            else None
        )
        status_transform = Reg_ohe.transform([[status]]) if status is not None else None
        reg_volume = (
            float(reg_qty) * float(reg_thick) * float(reg_width)
            if (reg_qty is not None and reg_thick is not None and reg_width is not None)
            else None
        )

        volume_box = (
            transform_single_value(reg_volume, lambda_dict["volume"])
            if reg_volume is not None
            else None
        )
        thick_box = (
            transform_single_value(reg_thick, lambda_dict["thickness"])
            if reg_thick is not None
            else None
        )
        width_box = (
            transform_single_value(reg_width, lambda_dict["width"])
            if reg_width is not None
            else None
        )
        qty_box = (
            transform_single_value(reg_qty, lambda_dict["quantity_tons"])
            if reg_qty is not None
            else None
        )
        ...
        reg_data = (
            np.append(reg_data, ohe_data, axis=1)
            if ohe_data is not None and app_label is not None
            else None
        )
        # st.write(reg_data)
        ...
        reg_scaled_data = (
            scale_reg.transform(reg_data) if reg_data is not None else None
        )
        # st.write(reg_scaled_data)
        ...
        if button and reg_data is not None:
            ...
            transformed_predict = (
                reverse_boxcox_transform(prediction, lambda_val)
                if reg_data is not None
                else None
            )
            rounded_prediction = round(transformed_predict[0], 2)
            st.success(
                f"Based on the input, the predicted price is  {rounded_prediction:.2f} USD"
            )
            st.snow()
